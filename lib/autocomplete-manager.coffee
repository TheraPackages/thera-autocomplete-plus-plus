...
class AutocompleteManager
  ...
  findSuggestions: (activatedManually) =>
    ...
    {line, prefix} = @getPrefix(@editor, bufferPosition)

    @getSuggestionsFromProviders({@editor, bufferPosition, scopeDescriptor, prefix, line, activatedManually})
  ...
  getPrefix: (editor, bufferPosition) ->
    ...
    prefix = @prefixRegex.exec(line)?[2] or ''
    return {line, prefix}
  ...
  replaceTextWithMatch: (suggestion) =>
    ...
    @editor.transact =>
      replacementPrefix = suggestion.internalReplacementPrefix ? suggestion.replacementPrefix
      for cursor in cursors
        endPosition = cursor.getBufferPosition()
        beginningPosition = [endPosition.row, endPosition.column - replacementPrefix.length]

        if @editor.getTextInBufferRange([beginningPosition, endPosition]) is replacementPrefix
          suffix = if @consumeSuffix then @getSuffix(@editor, endPosition, suggestion) else ''
          cursor.moveRight(suffix.length) if suffix.length
          cursor.selection.selectLeft(replacementPrefix.length + suffix.length)

          if suggestion.snippet? and @snippetsManager?
            @snippetsManager.insertSnippet(suggestion.snippet, @editor, cursor)
          else if suggestion.internalSnippet? and @snippetsManager?
            @snippetsManager.insertSnippet(suggestion.internalSnippet, @editor, cursor)
          else
